...
def verify_multiple_email(emailAddress, name):
    ...
    for email in emailAddress:
        if (verify_email(email)):
            result = {'Name': name, 'Email': email, 'Status': 'Verified'}
...
def generate_pattern(first_name='', last_name='', middle_name='', domain=''):
    ...
    if(middle_name != '' and first_name != ''):
        ...
        patterns += generate_three_name(first_name,
                                        last_name, middle_name, domain)
        patterns += generate_three_name(first_name,
                                        last_name, middle_initial, domain)
        patterns += generate_three_name(first_name,
                                        last_initial, middle_name, domain)
        patterns += generate_three_name(first_name,
                                        last_initial, middle_initial, domain)
        patterns += generate_three_name(first_initial,
                                        last_name, middle_name, domain)
        patterns += generate_three_name(first_initial,
                                        last_name, middle_initial, domain)
        patterns += generate_three_name(first_initial,
